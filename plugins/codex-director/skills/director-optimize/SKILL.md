...
description: Runs the Codex Director optimization lane for performance, cost, memory, bundle size, throughput, or latency work. Use when a Director worker brief explicitly invokes $codex-director:director-optimize for measurement-led improvement, bottleneck diagnosis, optimization patches, or performance verification.
metadata:
...
3. Report activation: `workflow-skill-loaded:$codex-director:director-optimize`, selected workflow/playbook `director-optimize`, top-level control loop, metric target, user-outcome fit, helper/subagent lane plan, and evidence contract.
...
1. Activate: restate optimization goal, metric, target threshold if any, selected workflow, top-level loop, user outcome, helper policy, and verification contract.
...
- If the work becomes broad or multi-lane, ask the Director to reroute to `$codex-director:director-orchestrate` or `$codex-director:director-dynamic-workflow`.
- Do not treat an improved metric as completion of a broader user outcome unless the metric is the final verification surface. Report checkpoint evidence and remaining proof separately when optimization supports a larger task.
