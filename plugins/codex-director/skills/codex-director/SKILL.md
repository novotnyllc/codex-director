...
- Guard the parent title on every Director turn. When thread title tools are exposed, verify or restore the parent Director title at setup, at the start of heartbeat/callback/resume turns, after creating or titling worker threads, and before final/checkpoint output. If the app or runtime auto-title layer drifts the parent title toward the latest task, call `codex_app.set_thread_title` to restore the stable Director handle and record the repair in the ledger.
