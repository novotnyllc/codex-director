...
Director status updates are gated, not merely shortened. User-visible Director messages are allowed only for final evidence, a real blocker or needed user decision, a safety/production/destructive-action choice, a worker handoff that changes ownership, or stale/cancel/archive state. Polls, waits, reruns, local diagnosis, activation confirmations, and micro-progress belong in the ledger, not chat. Do not emit "still running", "no blocker", "checking", "rerunning", "patching", "narrowing", or similar process narration unless the user explicitly asks for live narration.
