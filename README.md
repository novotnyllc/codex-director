...
```text
Use $codex-director:codex-director to start a Director for this project.
...
`@codex-director` is the plugin entrypoint, not a terminal command, and by itself may surface plugin context without loading the main skill. Use the canonical skill mention `$codex-director:codex-director`, or select one of the Codex Director starter prompts that includes it, when setting up the Director.

Plugin manifest `defaultPrompt` entries are composer starter prompts, not an instruction-enforcement surface. Keep them short, natural, built-in-style, and explicit about `$codex-director:codex-director`. Operational invocation rules belong in `skills/codex-director/SKILL.md` and the main skill's `agents/openai.yaml` activation policy, not in user-visible plugin descriptions.

The main Director skill activates from `$codex-director:codex-director`, Codex Director starter prompts that contain that canonical mention, or a plain request that the runtime maps to the skill. Do not rely on a bare `$codex-director` skill mention; plugin-provided skills are namespaced by the runtime. Workflow subskills remain explicit-invocation skills. The user starts the coordinator with `$codex-director:codex-director` or a Codex Director starter prompt; the Director then forces each worker lane to activate the right workflow by naming the matching subskill in the worker brief:
...
```text
Use $codex-director:codex-director to start a Director for this workspace. Treat admin/, website/, and ops/ as separate child repos under one project scope.
```
